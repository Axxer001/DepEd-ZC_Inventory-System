...
<head>
    ...
    <title>Verify PIN | DepEd Zamboanga City</title>
        ...
        <div class="flex flex-col items-center w-full max-w-md bg-white rounded-[2rem] shadow-none overflow-hidden animate-fade-up">
            ...
            <div class="p-8 md:p-10 pb-6 w-full">
                    <div class="mb-6 text-center">
                        <h2 class="text-2xl font-bold text-slate-800 tracking-tight">Verify PIN</h2>
                        <p class="text-slate-500 text-sm mt-1">Please enter the 6-digit PIN sent to your email.</p>
                    </div>

                    <form action="{{ route('password.verify.post') }}" method="POST" class="space-y-4 w-full">
                        @csrf
                ...
                <div class="mt-6 text-center">
                    <p class="text-sm text-slate-500">
                        <a href="{{ route('password.request') }}" class="text-[#c00000] font-bold hover:underline transition-colors">Resend PIN / Back</a>
                    </p>
                </div>
            </div>
