            ...
            <aside class="lg:col-span-3 flex flex-col gap-6 z-40 relative">
                <form action="{{ route('assets.photo.upload', $asset->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-visible flex flex-col relative" x-data="{ photoPreview: null, showPhotoConfirmModal: false, isHoveringImage: false }">
                    ...
                    <div class="p-5 space-y-5">
                        <div class="bg-gradient-to-r from-red-50 to-white border border-red-100 p-4 rounded-2xl shadow-sm relative overflow-hidden group">
                            ...
                            <div class="flex items-center gap-3 pl-1">
                                ...
                                <div>
                                    <p class="text-xs font-black text-slate-800 uppercase leading-tight group-hover:text-red-700 transition-colors">{{ $asset->office_school_name }}</p>
                                    <p class="text-[9px] font-bold text-slate-500 uppercase mt-0.5">Designated Custodian</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-y-6 gap-x-8 bg-slate-50 rounded-xl p-6 border border-slate-100">
                                <div>
                                    ...
                                    <p x-show="!isEditing" class="text-xs font-bold text-slate-800 mt-1 uppercase">{{ $asset->category_name }}</p>
                                    <input x-show="isEditing" type="text" value="{{ $asset->category_name }}" class="w-full mt-1.5 bg-white border-2 border-slate-100 rounded-xl px-4 py-2.5 text-xs font-black text-slate-700 uppercase focus:border-deped focus:ring-4 focus:ring-deped/10 outline-none transition-all shadow-sm hover:border-slate-200">
                                </div>
                                <div>
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Article / Item</p>
                                    <p x-show="!isEditing" class="text-xs font-bold text-slate-800 mt-1 uppercase">{{ $asset->item_name }}</p>
                                    <input x-show="isEditing" type="text" value="{{ $asset->item_name }}" class="w-full mt-1.5 bg-white border-2 border-slate-100 rounded-xl px-4 py-2.5 text-xs font-black text-slate-700 uppercase focus:border-deped focus:ring-4 focus:ring-deped/10 outline-none transition-all shadow-sm hover:border-slate-200">
                                </div>
                                <div>
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Quantity</p>
                                    <p x-show="!isEditing" class="text-xs font-black text-deped mt-1 uppercase">{{ $asset->quantity }} Unit(s)</p>
                                    <div x-show="isEditing" class="mt-1.5 relative flex items-center group">
                                        <input type="number" value="{{ $asset->quantity }}" class="w-full bg-deped_light/30 border-2 border-deped/20 rounded-xl pl-4 pr-16 py-2.5 text-sm font-black text-deped focus:border-deped focus:ring-4 focus:ring-deped/10 outline-none transition-all shadow-sm">
                                        <span class="absolute right-4 text-deped/60 font-black text-[10px] uppercase tracking-widest pointer-events-none">Unit(s)</span>
                                    </div>
                                </div>
                                <div>
                                    <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Brand / Make</p>
                                    <p x-show="!isEditing" class="text-xs font-bold text-slate-500 mt-1 uppercase italic">Not Specified</p>
                                    <input x-show="isEditing" type="text" placeholder="Not Specified" class="w-full mt-1.5 bg-white border-2 border-slate-100 rounded-xl px-4 py-2.5 text-xs font-black text-slate-700 uppercase focus:border-deped focus:ring-4 focus:ring-deped/10 outline-none transition-all shadow-sm hover:border-slate-200">
                                </div>
                                <div>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 bg-white rounded-xl p-6 border border-slate-200 shadow-sm">
                                ...
                                <div>
                                    ...
                                    <p x-show="!isEditing" class="text-lg font-black text-emerald-600 mt-0.5 tracking-tighter">₱ {{ number_format($asset->asset_cost, 2) }}</p>
                                    <div x-show="isEditing" class="mt-1.5 relative flex items-center group">
                                        <span class="absolute left-4 text-emerald-500 font-black text-[13px] group-focus-within:text-emerald-600 transition-colors pointer-events-none">₱</span>
                                        <input type="number" step="0.01" value="{{ $asset->asset_cost }}" class="w-full bg-emerald-50/50 border-2 border-emerald-100 rounded-xl pl-8 pr-4 py-2.5 text-sm font-black text-emerald-700 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/10 outline-none transition-all shadow-sm hover:border-emerald-200">
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <input x-show="isEditing" type="text" value="{{ $asset->source_name }}" class="w-full mt-1.5 bg-white border-2 border-slate-100 rounded-xl px-4 py-2.5 text-xs font-black text-slate-700 uppercase focus:border-deped focus:ring-4 focus:ring-deped/10 outline-none transition-all shadow-sm hover:border-slate-200">
                                </div>
                                <div>
                                    ...
                                    <input x-show="isEditing" type="text" value="{{ $asset->mode_of_acquisition }}" class="w-full mt-1.5 bg-white border-2 border-slate-100 rounded-xl px-4 py-2.5 text-xs font-black text-slate-700 uppercase focus:border-deped focus:ring-4 focus:ring-deped/10 outline-none transition-all shadow-sm hover:border-slate-200">
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div x-show="showTransferModal" x-cloak class="fixed inset-0 z-[100] flex items-center justify-center">
            ...
            <div x-show="showTransferModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-8 scale-95" x-transition:enter-end="opacity-100 translate-y-0 scale-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 scale-100" x-transition:leave-end="opacity-0 translate-y-8 scale-95" class="bg-white rounded-3xl shadow-2xl w-full max-w-xl mx-4 relative z-10 flex flex-col overflow-hidden border border-slate-100">
                ...
                <div class="p-6 space-y-6">
                    ...
                    {{-- Form Fields --}}
                    <div>
                        <label class="block text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2 ml-1">New School / Custodian <span class="text-deped">*</span></label>
                        <div class="relative group">
                            <select class="w-full appearance-none bg-white border-2 border-slate-200 rounded-xl pl-4 pr-10 py-3.5 text-xs font-black text-slate-700 uppercase focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all shadow-sm hover:border-slate-300 cursor-pointer">
                                <option value="" disabled selected>Select receiving school...</option>
                                <option value="1">Zamboanga City High School (Main)</option>
                                <option value="2">Ayala National High School</option>
                                <option value="3">Southcom National High School</option>
                                <option value="4">Culianan National High School</option>
                                <option value="5">Regional Office IX</option>
                            </select>
                    ...
                    <div class="grid grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="date" class="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 text-xs font-black text-slate-700 uppercase focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all shadow-sm hover:border-slate-300 cursor-pointer">
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <textarea rows="3" class="w-full bg-white border-2 border-slate-200 rounded-xl px-4 py-3 text-xs font-bold text-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 outline-none transition-all shadow-sm resize-none hover:border-slate-300" placeholder="State reason for transfer..."></textarea>
                    </div>
                ...
                <div class="bg-slate-50 border-t border-slate-100 p-6 flex items-center justify-end gap-3">
                    <button @click="showTransferModal = false" class="px-6 py-3 bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl text-xs font-black uppercase tracking-widest transition-colors shadow-sm active:scale-95">Cancel</button>
                    <button @click="showTransferModal = false" class="px-8 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-black uppercase tracking-widest shadow-lg shadow-blue-600/30 transition-all active:scale-95 flex items-center gap-2">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                        Confirm Transfer
                    </button>
                </div>
