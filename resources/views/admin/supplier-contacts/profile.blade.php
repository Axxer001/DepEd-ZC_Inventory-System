...
<head>
        ...
        tailwind.config = {
            theme: {
                extend: {
                    colors: {
                        deped: '#c00000',
                        supplier_primary: '#0f172a',
                        supplier_accent: '#3b82f6',
                        supplier_amber: '#f59e0b'
                    }
                }
            }
        }
    ...
    <style>
        ...
        .bento-glow::after { content: ''; position: absolute; top: -50%; left: -50%; width: 200%; height: 200%; background: radial-gradient(circle, rgba(59, 130, 246, 0.05) 0%, transparent 60%); pointer-events: none; }
        
        /* Dark Mode */
        html.dark body { background-color: #0b0f19; color: #f8fafc; }
        html.dark .glass-card { background: rgba(30, 41, 59, 0.6); border: 1px solid rgba(255, 255, 255, 0.05); }
    </style>
</head>
<body class="flex min-h-screen text-slate-800 dark:text-slate-100 overflow-hidden">
    ...
    <div class="flex-grow flex flex-col min-w-0 h-screen overflow-y-auto custom-scroll p-4 lg:p-8">
        ...
        {{-- Elegant Header Card --}}
        <header class="relative overflow-hidden bg-slate-900 text-white rounded-3xl p-8 mb-6 shadow-xl border border-slate-850 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6">
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(59,130,246,0.15),transparent_50%)] pointer-events-none"></div>
            
            <div class="flex items-center gap-6 relative z-10">
                <div class="w-16 h-16 bg-blue-500/10 border border-blue-500/30 rounded-2xl flex items-center justify-center shadow-inner shrink-0">
                ...
                <div>
                    <span class="text-[10px] font-bold text-blue-400 uppercase tracking-[0.2em]">Acquisition Representative</span>
                    <h1 class="text-3xl font-extrabold tracking-tight mt-1">{{ $contact->name }}</h1>
                    <div class="flex flex-wrap items-center gap-3 mt-3">
                        <span class="text-xs font-semibold bg-slate-800 text-slate-300 border border-slate-700 px-3 py-1 rounded-xl uppercase">{{ $contact->organization ?? 'External Provider' }}</span>
                        <span class="text-xs font-semibold bg-slate-800 text-slate-300 border border-slate-700 px-3 py-1 rounded-xl uppercase">{{ $contact->position ?? 'Personnel' }}</span>
                    </div>
                </div>
            </div>

            <div class="relative z-10 flex items-center gap-3 shrink-0">
                <a href="{{ route('admin.supplier_contacts') }}" class="px-5 py-3 bg-slate-800 hover:bg-slate-750 border border-slate-700 text-slate-200 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all duration-300 flex items-center gap-2 group shadow-lg">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 pb-12">
            ...
            <section class="lg:col-span-4 flex flex-col gap-6">
                <div class="glass-card rounded-[2.5rem] p-6 shadow-sm border border-slate-200/60 dark:border-slate-800 space-y-6">
                    <div class="pb-4 border-b border-slate-100 dark:border-slate-800">
                        <h3 class="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Provider Representative</h3>
                        <p class="text-lg font-black text-slate-900 dark:text-slate-100 uppercase mt-1">Personnel Info</p>
                    </div>

                    <div class="space-y-4">
                        <div class="p-4 bg-slate-55/40 dark:bg-slate-950/20 rounded-2xl border border-slate-100 dark:border-slate-850">
                            <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase block tracking-wider">Representative Name</span>
                            <span class="text-sm font-extrabold text-slate-800 dark:text-slate-200 mt-1 block uppercase">{{ $contact->name }}</span>
                        </div>
                        <div class="p-4 bg-slate-55/40 dark:bg-slate-950/20 rounded-2xl border border-slate-100 dark:border-slate-850">
                            <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase block tracking-wider">Assigned Position</span>
                        ...
                        <div class="p-4 bg-slate-55/40 dark:bg-slate-950/20 rounded-2xl border border-slate-100 dark:border-slate-850">
                            <span class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase block tracking-wider">Affiliated Supplier / Company</span>
                    ...
                    <div class="pt-6 border-t border-slate-100 dark:border-slate-800 space-y-3">
                        <a href="tel:{{ $contact->contact_number }}" class="w-full py-3 px-4 bg-blue-500 hover:bg-blue-600 text-white rounded-2xl text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-lg shadow-blue-500/10">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.94.725l.548 2.2a1 1 0 01-.321.988l-1.305.98a10.582 10.582 0 004.872 4.872l.98-1.305a1 1 0 01.988-.321l2.2.548a1 1 0 01.725.94V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                            Call: {{ $contact->contact_number ?: 'N/A' }}
                        </a>
                        <a href="mailto:{{ $contact->email }}" class="w-full py-3 px-4 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 rounded-2xl text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 border border-slate-200 dark:border-slate-700">
                            <svg class="w-4 h-4 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>
            ...
            <main class="lg:col-span-8 flex flex-col gap-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="glass-card bento-glow rounded-[2rem] p-6 shadow-sm border border-slate-200/60 dark:border-slate-800 flex justify-between items-center relative overflow-hidden">
                        <div class="relative z-10">
                            <p class="text-[10px] font-black text-slate-455 dark:text-slate-500 uppercase tracking-widest">Total Supplies Distributed</p>
                        ...
                        <div class="w-14 h-14 bg-blue-500/10 border border-blue-500/20 text-blue-500 rounded-2xl flex items-center justify-center shrink-0">
                    ...
                    <div class="glass-card bento-glow rounded-[2rem] p-6 shadow-sm border border-slate-200/60 dark:border-slate-800 flex justify-between items-center relative overflow-hidden">
                        <div class="relative z-10">
                            <p class="text-[10px] font-black text-slate-455 dark:text-slate-500 uppercase tracking-widest">Supplied Inventory Value</p>
                            <p class="text-4xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-2">₱ {{ number_format($stats->total_value, 2) }}</p>
                            <p class="text-[10.5px] font-semibold text-emerald-600/70 mt-2">Cumulative procurement cost stats</p>
                        </div>
                        <div class="w-14 h-14 bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 rounded-2xl flex items-center justify-center shrink-0">
                ...
                <div class="glass-card rounded-[2.5rem] p-6 shadow-sm border border-slate-200/60 dark:border-slate-800 flex flex-col flex-grow">
                    <div class="pb-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center mb-6">
                        <div>
                            <h3 class="text-xs font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">DepEd Inventory Integration</h3>
                            <p class="text-lg font-black text-slate-900 dark:text-slate-100 uppercase mt-1">Supplied Equipment Registry</p>
                        </div>
                        <span class="px-3 py-1 bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 text-[10px] font-black rounded-xl uppercase tracking-wider">{{ $assets->count() }} Items</span>
                    </div>
