    ...
    <div class="flex-grow flex flex-col min-w-0 h-screen lg:overflow-hidden overflow-y-auto custom-scroll p-4 lg:p-8 gap-5" x-data="{ activeTab: 'assets' }">
        ...
        <header class="bg-white rounded-2xl shadow-sm border border-slate-200 px-6 py-5 flex flex-col lg:flex-row lg:justify-between lg:items-center gap-4 sticky top-0 z-50 anim-0">
            ...
            <div class="flex items-center gap-3 shrink-0">
                @php
                    $canHardDeleteSource = auth()->check() && auth()->user()->isSuperAdmin() && auth()->user()->isMainSystem();
                    $sourceDeleteReasons = [];
                    if ($canHardDeleteSource) {
                        $sourceDeleteReasons = app(\App\Services\DeletionEligibilityService::class)->checkAcquisitionSource($source->id);
                    }
                    $sourceDeleteEligible = $canHardDeleteSource && empty($sourceDeleteReasons);
                @endphp
                ...
                @if($canHardDeleteSource)
                    @if($sourceDeleteEligible)
                        <form action="{{ route('admin.sources.hard_delete', $source->id) }}" method="POST" class="inline"
                              onsubmit="return confirm('PERMANENT DELETE\n\nThis action cannot be undone. The source record will be removed completely from the database.\n\nProceed?');">
                            @csrf
                            @method('DELETE')
                        ...
                        <div class="relative group/del">
                            ...
                            <div class="absolute bottom-full right-0 mb-2 w-72 bg-zinc-900 text-zinc-100 text-[10px] font-medium rounded-xl px-3 py-2.5 shadow-xl hidden group-hover/del:block z-50 leading-relaxed">
                                ...
                                <ul class="list-disc list-inside space-y-0.5">
                                    @foreach($sourceDeleteReasons as $reason)
                                        <li>{{ $reason }}</li>
                                    @endforeach
                                </ul>
                            </div>
